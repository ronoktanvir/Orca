...
def _prompt(digest: TraceDigest, cards: dict[str, BehaviorCard]) -> str:
    ...
    return (
        ...
        f"{cards_block}\n\n"
        "Each agent's 'reasoned:' lines are that worker's own words about what it was "
        "trying to do — use them to tell a DELEGATION error (wrong who/assignment) from "
        "an EXECUTION error (right assignment, but it missed a prerequisite step/tool). "
